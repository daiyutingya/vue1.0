<template>
    <div :style="{ width: sidebar.width + 'px' }" >
        <div class="menu_page_top rflex">
            <img :class="['logo', { closeLogo: !sidebar.open }]" :src="logo" alt="xxxx" />
            <span class="title" v-show="sidebar.open">xxxxxx<i>Admi</i></span>
        </div>
        <div class="menu_center is-scroll-left" >
            
            <el-menu :default-active="$route.path"   :show-timeout="200" :isCollapse="isCollapse" mode="vertical"
            :background-color="menuObj.bgColor"
                ...
                <template v-for="(item, index) in permission_routers">
                    <router-link v-if="item.noDropdown && !item.hidden" :to="item.path + '/' + item.children[0].path"
                        :key="index">
                    ...
                    " :key="index">
                        ...
                        <router-link v-for="(citem, cindex) in item.children" :key="cindex"
